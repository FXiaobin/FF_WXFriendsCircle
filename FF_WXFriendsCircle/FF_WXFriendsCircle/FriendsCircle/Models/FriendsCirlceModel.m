...
@implementation FriendsCirlceModel

-(CGFloat)cellHeight{
    if (_cellHeight == 0.0) {
        
        //用户信息高度
        CGFloat height = kSCALE(130.0);
        ...
        if (self.content.length) {
            ...
            CGFloat textH = [content boundingRectWithSize:CGSizeMake(CGRectGetWidth([UIScreen mainScreen].bounds) - kSCALE(60.0), CGFLOAT_MAX) options:NSStringDrawingUsesFontLeading | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesDeviceMetrics attributes:@{NSFontAttributeName : kFont(kSCALE(30.0)) , NSParagraphStyleAttributeName : para} context:nil].size.height;
        ...
        //底部按钮高度
        height += kSCALE(20.0) + kSCALE(80.0) + kSCALE(20.0);
