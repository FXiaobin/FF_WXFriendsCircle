//
//  WXFriendsCirlceViewController.m
//  FF_WXFriendsCircle
//
...
@interface WXFriendsCirlceViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation WXFriendsCirlceViewController
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    WXFriendsCircleModel *model = self.dataArr[indexPath.row];
    return model.cellHeight;
}

-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    
    WXFriendsCircleCell *cell = [tableView dequeueReusableCellWithIdentifier:@"WXFriendsCircleCell" forIndexPath:indexPath];
    ...
    cell.openBtnActionBlock = ^(WXFriendsCircleCell *currentCell,UIButton * _Nonnull sender, WXFriendsCircleModel * _Nonnull aModel,NSInteger tag) {
        if (tag == 0) {
            ...
            //弹窗时去除别的cell上的弹窗
            [WXFriendsCircleCell updateOperationViewShowStatusWithTableView:tableView currentCell:currentCell];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [WXFriendsCircleCell updateOperationViewShowStatusWithTableView:tableView currentCell:nil];
}

-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
 
    if (scrollView == self.tableView) {
        [WXFriendsCircleCell updateOperationViewShowStatusWithTableView:self.tableView currentCell:nil];
    }
}
...
-(void)loadData{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        WXFriendsCircleModel *model = [WXFriendsCircleModel new];
        model.name = dic[@"name"];
        ...
        if (i == 0 ) {
            model.comments = @[];
            model.likes = @[@"你让满山花开我就来~",@"小付",@"李浩"];
        }else if (i == 1 ) {
            model.likes = @[@"贾和超",@"曹建耀",@"小付",@"李浩",@"痛并快乐着！",@"那年花开正旺时~"];
...
-(UITableView *)tableView{
    if (_tableView == nil) {
        ...
        [_tableView registerClass:[WXFriendsCircleCell class] forCellReuseIdentifier:@"WXFriendsCircleCell"];
